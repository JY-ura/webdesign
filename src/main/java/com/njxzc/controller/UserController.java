...
public class UserController {
    ...
    **/
    ...
    public ModelAndView login(@ModelAttribute("user") MyUser user,HttpSession httpSession) {
    ...
    public ModelAndView select(MyUser user,Model model,HttpSession httpSession){
        ModelAndView mav = new ModelAndView("main");
        mav.addObject("u",httpSession.getAttribute("manager"));
        List<MyUser> list=userDao.selectUserByUname(user);
        model.addAttribute("uerlist",list);
        return mav;
    }
    @RequestMapping("/delete")
    public ModelAndView delete(MyUser user,HttpSession httpSession){
        ModelAndView mav = new ModelAndView("main");
        mav.addObject("u",httpSession.getAttribute("manager"));
        if(user.getUname() != null){
            userDao.delete(user);
            logger.info("删除成功，请刷新！");
            System.out.println("删除成功，请刷新！删除成功，请刷新！删除成功，请刷新！删除成功，请刷新！");
        }else{
            logger.info("该用户不存在");
        }
        return mav;
